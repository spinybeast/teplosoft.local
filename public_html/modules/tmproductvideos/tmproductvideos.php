...
class TmProductVideos extends Module
{
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->displayName = $this->l('TemplateMonster Product Videos');
		$this->description = $this->l('This module allow add videos to product.');
		$this->ps_versions_compliancy = array('min' => '1.5.1.0', 'max' => '1.6.1.10');
	...
	{
		$higher_ver = Tools::version_compare(_PS_VERSION_, '1.5.1.0', '>');
	...
	public function hookDisplayAdminProductsExtra()
	{
		if (Validate::isLoadedObject(new Product((int)Tools::getValue('id_product'))))
		{
			...
			if (Tools::version_compare(_PS_VERSION_, '1.6.0.0', '>')) {
				...
				return $this->display(__FILE__, 'views/templates/admin/tmproductvideos_tab_15.tpl');
			}
		}
		else
	...
	public function addVideo($id_product, $id_shop)
	{
		...
		foreach (Language::getLanguages() as $lang)
				if (trim(Tools::getValue('video_link_'.$lang['id_lang'])) != '')
					if (!Validate::isCleanHtml(Tools::getValue('video_name_'.$lang['id_lang']), (int)Configuration::get('PS_ALLOW_HTML_IFRAME'))
						|| !Validate::isCleanHtml(Tools::getValue('video_description_'.$lang['id_lang']), (int)Configuration::get('PS_ALLOW_HTML_IFRAME')))
					{
					...
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'product_video_lang
													(`id_video`, `id_shop`, `id_product`, `id_lang`, `link`, `name`, `description`, `sort_order`, `status`)
														VALUES (
															'.(int)$id_video.',
															'.(int)$id_shop.',
															'.(int)$id_product.',
