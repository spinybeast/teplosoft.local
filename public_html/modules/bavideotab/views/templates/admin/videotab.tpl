...
{if $editfrommodule == 1}
    <script src="{$url|escape:'htmlall':'UTF-8'}modules/bavideotab/views/js/showhidetab.js" type="text/javascript" charset="utf-8" async defer></script>
{/if}
<script src="{$url|escape:'htmlall':'UTF-8'}modules/bavideotab/views/js/videotab.js" type="text/javascript" charset="utf-8" async defer></script>
...
<div class="alert alert-warning alert_video" style="display: none;" >
    <button type="button" class="close" data-dismiss="alert">×</button>
            {l s='There is 1' mod='bavideotab'} <warning class=""></warning>
        <ul  id="seeMore">
            <li>{l s='You must save this product before adding specific videos' mod='bavideotab'}</li>
        </ul>
</div>
<div class="alert alert-danger alert_embed" style="display: none;">
    ...
    <br>
    <ol>
        <li>{l s='You can not leave the Embed field blank ' mod='bavideotab'}</li>
    </ol>
</div>
<div class="alert alert-danger alert_primary" style="display: none;">
    ...
    <br>
    <ol>
        <li>{l s='You must upload at least a file for default language' mod='bavideotab'}</li>
    </ol>
</div>
<div class="alert alert-danger alert_size" style="display: none;">
    ...
    <br>
    <ol>
        <li>{l s='Upload files are too large' mod='bavideotab'}</li>
    </ol>
</div>
<div class="alert alert-danger alert_select" style="display: none;">
    ...
    <br>
    <ol>
        <li>{l s='You need to select the file' mod='bavideotab'}</li>
    </ol>
</div>
<div class="alert alert-danger alert_type" style="display: none;">
    ...
    <br>
    <ol>
        <li>{l s='File is invalid' mod='bavideotab'}</li>
    </ol>
</div>
<div class="form-group {if $ver == 1}col-md-12{/if}">
    <h3 >{l s='Video Tab' mod='bavideotab'}</h3> 
        ...
        <div class="col-md-9">
            ...
            <p class="radio">
                <label id="upload" onclick="upload()">
                    ...
                    {l s='Upload Video' mod='bavideotab'}
                </label>
            </p>
        </div>
    </div>
    ...
    <div class="form-group" id="uploadvideo" style="{if empty($file)} display: none; {else} display: block; {/if}">
        ...
        <label class="control-label col-lg-2 required text-right text17">
            <span class="label-tooltip" data-toggle="tooltip" title="">
                {l s='Video files :' mod='bavideotab'}
            </span>
        </label>
        {foreach $languages key =key_lang item=value}
        <div class="translatable-field2 lang2-{$value['id_lang']|escape:'htmlall':'UTF-8'}" style="{if $value['checked']==1}display: block;{/if}{if $value['checked']!=1}display: none;{/if}">
            <div class="col-lg-4">
                ...
                <button  class="btn {if $ver == 1}btn-primary {else}btn-default{/if}" data-style="expand-right" data-size="s" type="button" id="text_file" onclick="textfile({$value['id_lang']|escape:'htmlall':'UTF-8'})" ><span class="ladda-label">
                    <i class="icon-plus-sign"></i> {l s='Add file' mod='bavideotab'}    
                            ...
                            {if $value1['type']|escape:'htmlall':'UTF-8'== 1}
                                {if $ver == 1}
                                    <a href="javascript:void(0);" onclick="ConfirmDelete({$value1['id_product']|escape:'htmlall':'UTF-8'},{$value1['type']|escape:'htmlall':'UTF-8'},{$value1['id_lang']|escape:'htmlall':'UTF-8'})" data-toggle="tooltip" class="btn btn-invisible delete" data-original-title="Video delete this product.">
                                      <i class="material-icons">{l s='delete' mod='bavideotab'}</i>
                                    </a>
                                    {else}
    ...
    <div class="panel-footer">
        {if $editfrommodule == 1}
            <a href="{$url2|escape:'htmlall':'UTF-8'}" class="btn {if $ver == 1}btn-primary {else}btn-default{/if}" id="" ><i class="process-icon-cancel"></i>  {l s='Cancel' mod='bavideotab'}</a>
            <button type="button" name="save" class="btn {if $ver == 1}btn-primary {else}btn-default{/if} pull-right " id="save_edit" onclick="savevideoedit()"> <i class="process-icon-save"></i> {l s='Save' mod='bavideotab'} </button>
        {else}
            <a href="{if $ver == 1}{$link->getAdminLink('AdminProducts')|escape:'htmlall':'UTF-8'} {else}{$cancel|escape:'htmlall':'UTF-8'} {/if}" class="btn {if $ver == 1}btn-primary {else}btn-default{/if} " ><i class="process-icon-cancel"></i>  {l s='Cancel' mod='bavideotab'}</a>
            <button type="button" name="save" class="btn {if $ver == 1}btn-primary {else}btn-default{/if}  pull-right " id="save" onclick="savevideo()"> <i class="process-icon-save"></i> {l s='Save' mod='bavideotab'}</button>    
        {/if}    
    </div>
</div>