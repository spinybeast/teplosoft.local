...
class BaVideoTab extends Module
{
    public $demoMode = false;
    public function __construct()
    {
        $this->name = "bavideotab";
    ...
    public function hookdisplayAdminProductsExtra($params)
    {
        ...
        ."&configure=bavideotab&tab_module=shipping_logistics&module_name=bavideotab&task=VideoList";
    ...
    public function resetVideo()
    {
        ...
        foreach ($search_field as $v) {
            $this->context->cookie->{'bavideotaburl_videoFilter_'.$v} = null;
        }
        return true;
    }
    ...
    public function delete()
    {
        ...
        $db = Db::getInstance(_PS_USE_SQL_SLAVE_);
        $idArray = Tools::getValue('bavideotaburl_videoBox');
        $id_shop=($this->context->shop->id);
    ...
    public function caseVideo()
    {
        ...
        $id_shop=($this->context->shop->id);
        if (Tools::isSubmit('submitResetbavideotaburl_video')) {
            $this->resetVideo();
            Tools::redirectAdmin($adminControllers.$token.$configAndTask.'&task=VideoList');
        } elseif (Tools::isSubmit('deletebavideotaburl_video')) {
            $id_video =  Tools::getValue('id_video');
            $this->deletea();
            Tools::redirectAdmin($adminControllers.$token.$configAndTask.'&task=VideoList');
        } elseif (Tools::isSubmit('updatebavideotaburl_video')) {
            $db = Db::getInstance(_PS_USE_SQL_SLAVE_);
            $id_video=Tools::getValue('id_video');
            $sql="SELECT * FROM "._DB_PREFIX_."url_video WHERE id_video='".$id_video."' AND id_store='".$id_shop."' ";
            $data = $db->ExecuteS($sql);
        ...
        } elseif (Tools::isSubmit('submitBulkdeletebavideotaburl_video')) {
            $this->delete();
            Tools::redirectAdmin($adminControllers.$token.$configAndTask.'&task=VideoList');
        }
    }
